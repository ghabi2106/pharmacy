...
class Pharmaceutical
{
  ...
  public function getPharmaceuticalCatById($id)
  {
    $this->db->query('SELECT 
    pharmaceuticals.*,
  ...
  public function get_search($category_id, $search_str, $limit_num)
  {
    if ($search_str != null && $category_id != 0) {
      $this->db->query("SELECT * FROM pharmaceuticals WHERE category_id = :category_id 
        && title LIKE '%{$search_str}%' " . $limit_num);
      $this->db->bind(':category_id', $category_id);
    } else if ($search_str != null) {
      $this->db->query("SELECT * FROM pharmaceuticals WHERE title LIKE '%{$search_str}%' " . $limit_num);
    } else if ($category_id != 0) {
      $this->db->query('SELECT * FROM pharmaceuticals WHERE category_id = :category_id ' . $limit_num);
      $this->db->bind(':category_id', $category_id);
    } else {
      $this->db->query('SELECT * FROM pharmaceuticals ' . $limit_num);
    }
